...
namespace StorybrewScripts
{
    public class Karaoke : StoryboardObjectGenerator
    {
        ...
        public void generateLyrics(FontGenerator font, SubtitleSet subtitles, string layerName, bool additive)
        {
            ...
            var subY = new List<float>(){360, 380, 400};
            var count = 0;
            foreach (var subtitleLine in subtitles.Lines)
            {
                var letterY = subY[count];
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    var matches = regex.Matches(line.ToUpper());
                    ...
                    foreach (Match match in matches)
                    {
                        ...
                        foreach (var letter in text)
                        {
                            ...
                            if (!texture.IsEmpty)
                            {
                                ...
                                sprite.MoveY(OsbEasing.OutBack, subtitleLine.StartTime - 200 + delay, subtitleLine.EndTime, position.Y + 10, position.Y);
                                if (additive)
